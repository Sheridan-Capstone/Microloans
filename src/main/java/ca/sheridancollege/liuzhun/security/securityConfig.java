...
public class securityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers(HttpMethod.POST, "/register").permitAll().antMatchers("/secure/**")
				.hasAnyRole("DONOR", "STUDENT", "ADMIN").antMatchers("/demo/checkout/api/paypal/order/create/")
				.permitAll().antMatchers("/demo/checkout/api/paypal/order/capture/").permitAll()
	...
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
		auth.userDetailsService(donorDetailsServiceImple).passwordEncoder(passwordEncoder());

		auth.userDetailsService(studentDetailsServiceImple).passwordEncoder(passwordEncoder());

		auth.userDetailsService(adminDetailsServiceImple).passwordEncoder(passwordEncoder());
