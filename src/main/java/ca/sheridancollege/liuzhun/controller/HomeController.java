...
public class HomeController {
	...
	@GetMapping("/secure")
	public String secureIndex(Model model, Authentication authentication, RedirectAttributes redirectAttributes) {
		...
		if (roleList.get(0).toString().equals("ROLE_DONOR")) {
		...
		} else if (roleList.get(0).toString().equals("ROLE_STUDENT") && (studentRepository.findByEmail(name).getApproval() == 1)) {
			...
			redirectAttributes.addAttribute("message", "Account Has Not Been Approved By Admin, Please Wait For Approval");
	...
	public String doRegistration(@RequestParam String username, @RequestParam String firstname,
			@RequestParam String lastname, @RequestParam String email, @RequestParam String password, @RequestParam String account, @RequestParam String program, @RequestParam String postSecondaryName, @RequestParam String highSchool, @RequestParam String story, @RequestParam String city) {
		...
		if (account.equals("donor")) {;
			...
			Student student = new Student(username, firstname, lastname, email, encodePassword(password), true, highSchool, postSecondaryName, city, program, story);
			student.setUsername(username);
			student.setEmail(email);
			student.setEncryptedPassword(encodePassword(password));
			student.setEnabled(true);
			student.setApproval(0);
